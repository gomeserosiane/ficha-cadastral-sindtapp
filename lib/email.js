...
export function getConfiguredFinalRecipients() {
  const listValue = process.env.FINAL_DOCUMENT_EMAILS || "";
  const listEmails = listValue
    .split(",")
...
export function getFinalRecipients({ metadata = {} } = {}) {
  const metadataRecipients = Array.isArray(metadata.finalRecipients) ? metadata.finalRecipients : [];
  const configuredRecipients = getConfiguredFinalRecipients();

  return [...new Set(
    [...metadataRecipients, ...configuredRecipients]
      .map((email) => String(email || "").trim().toLowerCase())
      .filter(isValidEmail)
  )].slice(0, 2);
}
...
export function getNewProponentNotificationRecipient() {
  const email = String(process.env.FINAL_DOCUMENT_EMAIL_1 || "").trim().toLowerCase();
  return isValidEmail(email) ? email : "";
}

export async function sendNewProponentNotificationEmail({ proponenteName, proponenteCpf, proponenteNascimento }) {
  const to = getNewProponentNotificationRecipient();

  // Essa notificação é extra e não interfere no fluxo atual da Assinafy.
  // Se FINAL_DOCUMENT_EMAIL_1 não existir, o formulário continua sendo enviado normalmente.
  if (!to) {
    return { skipped: true, reason: "FINAL_DOCUMENT_EMAIL_1 não configurado." };
  }
  ...
  if (!smtpUser || !smtpPass || !emailFrom) {
    return { skipped: true, reason: "SMTP_USER, SMTP_PASS ou EMAIL_FROM não configurado." };
  }
  ...
  await transporter.sendMail({
    ...
    text: [
      ...
      `Nome: ${proponenteName || "Não informado"}`,
      `CPF: ${proponenteCpf || "Não informado"}`,
      `Data de nascimento: ${proponenteNascimento || "Não informado"}`,
    ].join("\n"),
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827;">
        <h2>Novo proponente cadastrado!!!</h2>
        <p><strong>Nome:</strong> ${escapeHtml(proponenteName || "Não informado")}</p>
        <p><strong>CPF:</strong> ${escapeHtml(proponenteCpf || "Não informado")}</p>
        <p><strong>Data de nascimento:</strong> ${escapeHtml(proponenteNascimento || "Não informado")}</p>
      </div>
    `,
  });
